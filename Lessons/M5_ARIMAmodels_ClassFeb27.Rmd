...
#we will mostly use fitted that can be found by using Modell 111$ in the console.
plot_grid(
  ...
  nrow=1
) #Acf and PACF still present some outside bcz it is our first model.

#Now let's try ARIMA(0,1,1) #this means no air component.
...
print(Model_211)
compare_aic <- data.frame(compare_aic,Model_211$aic) #drift is the intercept term. It should be close to zero.
...
#this looks to be the best fit. Then compare the AIC. You can
...
print(Model_autofit) #this do all the previous steps at once.
#if your AIC is not decreasing, it is time to stop instead of adding other steps. 
...
autoplot(deseasonal_residential_price,series="Observed")+
  autolayer(Model_111$fitted, series="Arrima(111)")+
  autolayer(Model_211$fitted, series="Arrima(211)")+
  autolayer(Model_212$fitted, series="Arrima(212)")+
  autolayer(Model_011$fitted, series="Arrima(011)")
