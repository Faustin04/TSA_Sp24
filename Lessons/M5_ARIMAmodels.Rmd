...
knitr::opts_chunk$set(echo = TRUE,tidy.opts=list(width.cutoff=80), tidy=FALSE) #this is a new ts
```
...
electricity_price_processed <-
  electricity_price %>%
  mutate( Month = my(Month) ) %>% #create date columns and rename the columns. 
...
TS_Plot <- 
  ggplot(electricity_price_processed, aes(x=Month, y=Residential)) + #ggplot when data are not ts.
      geom_line()
plot(TS_Plot)
...
electricity_price_processed <-
  electricity_price_processed %>% 
  arrange(Month) #arrange by moth and plot again to see the rearanged data
...
#ACF and PACF plots : we see trend and seasonality in the plot. 
