...
#modelling a trend is to come up with *an equestion* that demonstrate what you want to do. 
...
for(i in 1:nhydro){
  print(ggplot(inflow_data, aes(x=my_date, y=inflow_data[,(1+i)])) + # +1 to skip one line (the date for this example). if you want to do plot with ggplot, use the *print* command.
...
tail(inflow_data) #this is helping to visualize the trend. the zeros can be removed before any other analysis. These are missing data, so theyr have to be removed. 
...
inflow_data <- inflow_data[1:(nobs-12),] #elimination of the lines (years) with zeros.
...
```{r}
ts_inflow_data <- ts(inflow_data[,2:(2+nhydro-1)], start=c(1931,1), frequency=12) #monthly data
#note that we are only transforming columns with inflow data, not the date columns  #start=my_date[1],end=my_date[nobs],
#nhydro-1 refers to the number of colunms minus one just to extract the one we want to use. 
...
iHP <- 4 #hive us HP4 with strong component. iHP is just an index.

#Fit a linear trend to TS of iHP
linear_trend <- lm(inflow_data[,(iHP+1)]~t) #plus one to call our colunm
...
beta0 <- linear_trend$coefficients[1] #for beta1 ans can be checked in the conole before running
...
ggplot(df_detrend,aes(x=date))+
  ...
  geom_line(aes(y=detrend),color="blue") #check where I made a mistake with prof file.
