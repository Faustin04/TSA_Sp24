...
raw_inflow_data <- read.table(file="/Users/faustinkambale/Library/CloudStorage/OneDrive-DukeUniversity/Spring 2024 classes/Time Series 4 Energy/TSA_Sp24_New/Data/inflowtimeseries.txt",header=FALSE,skip=0)
...
iHP=1
decompose_inflow_data <- decompose(ts_inflow_data[,iHP],"additive") #use either additive or multiplicative. The latter if we believe the trend is function of time. 
#should start with additive and try the multiplicative. 
#stl also do the job, but cannot do the multiplicative.
...
#Note random series has some missing values, that is why we got NAs. We are missing 
#some observations. 
#summary function gives us how many NAs we have. 
...
autoplot(inflow_random)+
  autolayer(tsclean(inflow_random))#tsclean removes the outliers.
...
df_inflow_random <- na.omit(df_inflow_random) # removes na observations.
...
ggplot(df_inflow_random, aes(x=month, y=inflow)) +
            geom_boxplot() #these are not outliers. they should be treated separately to make sure we know what they are. 
...
```{r}
#Just find extreme value (identify the outlier)
...
#use tsclean if you are confident with your analysis. 
